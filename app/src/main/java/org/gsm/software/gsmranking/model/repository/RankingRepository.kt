...
class RankingRepository @Inject constructor (val api : ApolloClient) {
    fun getRanking(generation : Int, criteria : String) : ApolloCall<RankingResultQuery.Data>{
        return api.query(RankingResultQuery(criteria = criteria , 0, page = 1, generation))
    }
}